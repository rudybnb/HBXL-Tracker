...
app.use('/api/telegram', async (req, res, next) => {
  try {
    const pythonApiUrl = `${process.env.PYTHON_API_URL || 'http://localhost:8000'}${req.originalUrl}`;
    ...
    const response = await fetch(pythonApiUrl, fetchOptions);
    ...
    res.status(503).json({
      ...
      message: 'The Telegram API service is currently unavailable. Please ensure the Python API is running on port 8000.'
    });
  }
});
...
(async () => {
  // Initialize Manus-n8n schema (adds new columns if needed)
  const { initManusSchema } = await import('./db');
  await initManusSchema();
  ...
  setInterval(async () => {
    try {
      await telegramService.getRecentMessages();
    } catch (error) {
