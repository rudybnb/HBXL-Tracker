  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        } else if (username === "Test Contractor" && password === "test123") {
          // Test Account Fallback
          localStorage.setItem('userRole', 'contractor');
          localStorage.setItem('isLoggedIn', 'true');
          localStorage.setItem('contractorName', 'Test Contractor');
          localStorage.setItem('contractorId', 'test-contractor-uuid');
          console.log('✅ Test Contractor login successful (Fallback)');
          window.location.href = '/';
          toast({
            title: "Login Successful",
            description: "Welcome back, Test Contractor!",
          });
        } else {
          toast({
            title: "Login Failed",
